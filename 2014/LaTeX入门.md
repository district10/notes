...
\end{enumerate}
% counter
\item This is \# \theenumi % enumi, enumii, enumiii, enumiv
\arabic{enumi}
\alph{enumi}
\roman{enumi}
\Alph{enumi}
...
% 或者使用 enumitem 包
\usepackage{enumitem}
\begin{enumerate}[itemsep=0pt,parset=0pt,label=(\arabic*)]
\item 中文
\item English
\end{enumerate}
```
...
前沿\parbox{2em}{不搭后语}。
\parbox[t]{2em}{Content} % top
\parbox[b]{2em}{Content} % bottom
\parbox[c]{2em}{Content} % center
...
请直接编译主文档：xelatex main.tex
```


```tex
\usepackage{syntonly} % syntonly 包（syntax only）
\syntaxonly
```
